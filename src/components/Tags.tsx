...
const Tags = ({ tags = [], onTagsChange, tagOptions }: ITags) => {
  ...
  const onAddTag = (tag: string) =>
    onTagsChange([tag, ...tags.filter((t: string) => t !== tag)]);
  const onRemoveTag = (tag: string) =>
    onTagsChange(tags.filter((t: string) => t !== tag));
...
//   margin: 5px;
// `;
const Select = styled.select<{ arrowHexColor: string }>`
  appearance: none;
